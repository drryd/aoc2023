...
public class Hand : IComparable<Hand>
{
    ...
    private static HandType ComputeHandType(string hand)
    {
        ...
        foreach ( var card in hand )
        {
            ...
            {
                cardCount[card]++;
                if ( cardCount[card] > highest_count )
                {
                    highest_count = cardCount[card];
                }
            }
            else
            {
        ...
        // Add the jokers to whatever our highest count is so far.
        foreach ( var entry in cardCount )
        {
            if ( entry.Value == highest_count )
            {
                cardCount[entry.Key] += num_jokers;
            }
        }
