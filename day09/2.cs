...
foreach ( var line in lines )
{
    ...
    sum_extrapolated_values += FindPrevItemInSequence(sequence);
}

Console.WriteLine(sum_extrapolated_values);

int FindPrevItemInSequence( List<int> sequence )
{
    ...
    int first_element_in_last_sequence = 0;
    int running_counter = 0;

    // At this point, all elements in the last sequence are equal.
    for ( var i = sequences.Count - 1; i >= 0; i-- )
    {
        var first_element_in_this_sequence = sequences[i][0];
        running_counter = first_element_in_this_sequence - running_counter;
        first_element_in_last_sequence = first_element_in_this_sequence;
    }
